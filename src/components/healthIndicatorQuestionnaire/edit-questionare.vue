<template>
  ...
  <div class="clearfix fixed-sub-header">
    ...
    <div class="pdf-title">{{$t('healthIndicatorQuestionnaire.pdfTitle', {country: countrySummary.countryName})}}</div>
    ...
    <div class="float-right button-container">
      ...
      <span v-if="status === 'REVIEW_PENDING' && isAdmin">
        <span v-if="showEdit"><button class="submit-btn btn btn-primary" @click="saveData('saveCorrection')"> <i class="fa fa-floppy-o" aria-hidden="true"></i>{{ $t('healthIndicatorQuestionnaire.save') }}</button></span>
        <span v-if="showEdit"><button class="submit-btn btn btn-green" @click="validate('publish')">
          <i class="fa fa-check" aria-hidden="true"></i>{{ $t('healthIndicatorQuestionnaire.publish') }}</button></span>
        <span v-if="showEdit"><button class="submit-btn btn btn-red" @click="reject()"><i class="fa fa-times" aria-hidden="true"></i>{{ $t('healthIndicatorQuestionnaire.reject') }}</button></span>
        <v-dialog/>
      </span>
      <span v-else>
  ...
  <form class="health-indicator-questionnaire-contact-info clearfix sub-content">
    ...
    <div class="box contact-info">
      <div class="row">
        <div class="collected-date column-33percent">
          ...
          <span class="fa fa-eye publish" :title="$t('healthIndicatorQuestionnaire.note1')"></span>
          <input type="text" :disabled="!showEdit" :class="errors.has('collectedDate') ? 'has-error form-control' : 'form-control'" name='collectedDate' id="date" :placeholder="$t('healthIndicatorQuestionnaire.contactForm.dateFormat')" v-model="countrySummary.collectedDate" v-validate.disable="'required|date_format:DD-MM-YYYY|date_between:01-01-2010,'+today+',true'" :title="$t('healthIndicatorQuestionnaire.contactForm.hoverText.date')">
          <div v-if="errors.has('collectedDate')" class="error-info">{{ $t('healthIndicatorQuestionnaire.contactForm.error.wrongDate') }}</div>
        </div>
      ...
      <div class="row">
        <div class="form-group column-33percent">
          ...
          <span class="mandatory-field">*</span>
          <span class="fa fa-eye-slash publish" :title="$t('healthIndicatorQuestionnaire.note2')"></span>
          <input type="text" :disabled="!showEdit" name='feederName' :class="errors.has('feederName') ? 'has-error form-control' : 'form-control'" id="nameofPersonEnteringData" v-validate.disable="'required'" v-model="countrySummary.dataFeederName"
                 :title="errors.has('feederName') ? $t('healthIndicatorQuestionnaire.contactForm.error.nameOfPersonEnteringData') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.nameOfPersonEnteringData')">
        </div>
        <div class="form-group column-33percent">
          ...
          <input type="text" :disabled="!showEdit" name="feederRole" :class="errors.has('feederRole') ? 'has-error form-control' : 'form-control'" id="roleOfPersonEnteringData" v-validate.disable="'required'" v-model="countrySummary.dataFeederRole"
                 :title="errors.has('feederRole') ? $t('healthIndicatorQuestionnaire.contactForm.error.roleOfThePersonEnteringData') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.roleOfThePersonEnteringData')">
        </div>
        <div class="form-group column-33percent">
          <label for="emailOfPersonEnteringData">{{ $t('healthIndicatorQuestionnaire.contactForm.emailOfThePersonEnteringData') }}</label>
          <span class="mandatory-field">*</span>
          <span class="fa fa-eye-slash publish" :title="$t('healthIndicatorQuestionnaire.note2')"></span>
          <input type="email" :disabled="!showEdit" name='feederEmail' :class="errors.has('feederEmail') ? 'has-error form-control' : 'form-control'" id="emailOfPersonEnteringData" v-validate.disable="'required|email'" v-model="countrySummary.dataFeederEmail" :title="$t('healthIndicatorQuestionnaire.contactForm.hoverText.email')">
          <div v-show="errors.has('feederEmail')" class="error-info">{{ $t('healthIndicatorQuestionnaire.contactForm.error.email') }}</div>
        </div>
      </div>
      <div class="row">
        <div class="form-group column-33percent">
          ...
          <input type="text" :disabled="!showEdit" class="form-control" id="nameofPersonApprovedData"  name="approvername" v-model="countrySummary.dataApproverName" :class="errors.has('approvername') ? 'has-error form-control' : 'form-control'" v-validate.disable="'required'"
                 :title="errors.has('approvername') ? $t('healthIndicatorQuestionnaire.contactForm.error.nameOfTheApprover') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.nameOfTheApprover')">
        </div>
        <div class="form-group column-33percent">
          ...
          <input type="text" :disabled="!showEdit" class="form-control" id="roleofPersonApprovedData" name="approverrole" v-model="countrySummary.dataApproverRole" :class="errors.has('approverrole') ? 'has-error form-control' : 'form-control'" v-validate.disable="'required'"
                 :title="errors.has('approverrole') ? $t('healthIndicatorQuestionnaire.contactForm.error.roleOfTheApprover') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.roleOfTheApprover')">
        </div>
        <div class="form-group column-33percent">
          ...
          <input type="email" :disabled="!showEdit" class="form-control" id="emailofPersonApprovedData" name="approveremail" v-model="countrySummary.dataApproverEmail" :class="errors.has('approveremail') ? 'has-error form-control' : 'form-control'" v-validate.disable="'required|email'"
                 :title="$t('healthIndicatorQuestionnaire.contactForm.hoverText.email')">
          <div v-show="errors.has('approveremail')" class="error-info">{{ $t('healthIndicatorQuestionnaire.contactForm.error.email') }}</div>
        </div>
      </div>
      <div class="row">
        <div class="form-group column-33percent">
          ...
          <input type="text" :disabled="!showEdit" name="countryContact" :class="errors.has('countryContact') ? 'has-error custom-form-control' : 'custom-form-control'" id="nameofCountryContact" v-validate.disable="'required'" v-model="countrySummary.contactName"
            :title="errors.has('countryContact') ? $t('healthIndicatorQuestionnaire.contactForm.error.nameOfTheCountryContact') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.nameOfTheCountryContact')">
        </div>
        <div class="form-group column-33percent">
          ...
          <input type="text" :disabled="!showEdit" name="contactRole" :class="errors.has('contactRole') ? 'has-error form-control' : 'form-control'" id="roleofCountryContact" v-validate.disable="'required'"
                 :title="errors.has('contactRole') ? $t('healthIndicatorQuestionnaire.contactForm.error.roleOfTheCountryContact') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.roleOfTheCountryContact')"
        ...
        <div class="form-group column-33percent">
          ...
          <span class="mandatory-field">*</span>
          <span class="fa fa-eye publish" :title="$t('healthIndicatorQuestionnaire.note1')"></span>
          <input type="email" :disabled="!showEdit" name="contactEmail" :class="errors.has('contactEmail') ? 'has-error form-control' : 'form-control'" id="emailofCountryContact" v-validate.disable="'required|email'" v-model="countrySummary.contactEmail" :title="$t('healthIndicatorQuestionnaire.contactForm.hoverText.email')">
          <div v-show="errors.has('contactEmail')" class="error-info">{{ $t('healthIndicatorQuestionnaire.contactForm.error.email') }}</div>
        </div>
      </div>
      <div class="row">
        <div class="column-66percent">
          ...
          <textarea name="countrySummary" rows="5" :disabled="!showEdit" :class="errors.has('countrySummary') ? 'has-error form-control custom-width' : 'form-control custom-width'" id="countrySummary" v-model="countrySummary.summary" v-validate.disable="'required'"
            :title="errors.has('countrySummary') ? $t('healthIndicatorQuestionnaire.contactForm.error.countrySummary') : $t('healthIndicatorQuestionnaire.contactForm.error.countrySummary')"></textarea>
        </div>
        <div class="column-33percent">
          ...
          <input type="text" :disabled="!showEdit" id="Organisation" name="Organisation" v-validate.disable="'required'"
                 ...
                 :title="errors.has('Organisation') ? $t('healthIndicatorQuestionnaire.contactForm.error.organisationOfTheCountryContact') : $t('healthIndicatorQuestionnaire.contactForm.hoverText.organisationOfTheCountryContact')">
        </div>
      </div>
    </div>
    <div class="health-indicator-questionnaire-contact-info-heading header-bold">{{ $t('healthIndicatorQuestionnaire.resourceForm.resourceInformation') }}
      <span class="fa fa-eye health-indicator-questionnaire-contact-info-publish" :title="$t('healthIndicatorQuestionnaire.note1')"></span>
    ...
    <div class="box resources">
      ...
      <div class="row">
        <div class="form-group column-33percent">
          ...
          <div v-if="errors.has('resource1')" class="error-info resource">{{ $t('healthIndicatorQuestionnaire.resourceForm.errorMessage') }}</div>
        </div>
        <div class="form-group column-33percent">
          ...
          <div v-if="errors.has('resource2')" class="error-info resource">{{ $t('healthIndicatorQuestionnaire.resourceForm.errorMessage') }}</div>
        </div>
        <div class="form-group column-33percent">
          ...
          <div v-if="errors.has('resource3')" class="error-info resource">{{ $t('healthIndicatorQuestionnaire.resourceForm.errorMessage') }}</div>
        </div>
      </div>
      <div class="row column-66percent">
        <div class="form-group column-50percent">
          ...
          <div v-if="errors.has('resource4')" class="error-info resource">{{ $t('healthIndicatorQuestionnaire.resourceForm.errorMessage') }}</div>
        </div>
        <div class="form-group column-50percent">
          ...
          <div v-if="errors.has('resource5')" class="error-info resource">{{ $t('healthIndicatorQuestionnaire.resourceForm.errorMessage') }}</div>
        </div>
      </div>
    </div>
    <div class="health-indicator-questionnaire-contact-info-heading header-bold">{{ $t('healthIndicatorQuestionnaire.indicatorDetails') }}
      <span :title="$t('healthIndicatorQuestionnaire.note3')" class="fa fa-eye health-indicator-questionnaire-contact-info-publish"></span>
    </div>
            ...
            <div class="" data-header="Indicators">
              <div v-for="indicator in category.indicators" :class="(errors.has('indicator_'+indicator.indicatorId+'_rationale')|| errors.has(indicator.indicatorId))? 'has-error indicator-items' : 'indicator-items'">
                ...
                <div class="row description_container">
                  ...
                  <div class="pdf-rationale-or-supporting-text"><p>{{healthIndicators[indicator.indicatorId].supportingText}}</p></div>
                  ...
                  <div v-show="(errors.has('indicator_'+indicator.indicatorId+'_rationale') || errors.has(indicator.indicatorId))" class="error-info">{{ $t('healthIndicatorQuestionnaire.indicatorScoreError') }}</div>
                </div>
              </div>
            </div>
          </div>
